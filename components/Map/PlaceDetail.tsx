...
import { BsClock, BsGeoAlt, BsTelephoneFill, BsXLg } from "react-icons/bs";
...
const PlaceDetail = ({
  ...
  return (
    <ScrollArea className="relative h-fit max-h-screen w-full bg-background md:max-h-[calc(100vh-80px)] md:rounded-2xl">
      <ActionIcon
        className="absolute right-3 top-3 z-50 rounded-full border"
        variant="filled"
        onClick={onClose}
      >
        <BsXLg />
      ...
      <div className="space-y-1 px-6 py-4">
        ...
        {!!(place.rating && place.user_ratings_total) ? (
          <UserRating rating={place.rating} total={place.user_ratings_total} />
        ) : (
